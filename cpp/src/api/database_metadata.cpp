...
ODBC_API nanodbc::result* get_database_meta_data_schemas(nanodbc::connection* conn, NativeError* error) {
	LOG_DEBUG("Getting database schemas from connection: {}", reinterpret_cast<uintptr_t>(conn));
	init_error(error);
	try {
		if (!conn) {
		...
		const DatabaseMetaData database_meta_data(*conn);
		auto result = database_meta_data.getSchemas();
...
ODBC_API nanodbc::result* get_database_meta_data_catalogs(nanodbc::connection* conn, NativeError* error) {
	LOG_DEBUG("Getting database catalogs from connection: {}", reinterpret_cast<uintptr_t>(conn));
	init_error(error);
	try {
		if (!conn) {
			LOG_ERROR("Connection pointer is null, cannot get tables");
		...
		const DatabaseMetaData database_meta_data(*conn);
		auto result = database_meta_data.getCatalogs();
		auto result_ptr = new nanodbc::result(result);
		LOG_DEBUG("ResultSet created successfully: {}", reinterpret_cast<uintptr_t>(result_ptr));
		return result_ptr;
	} catch (const nanodbc::database_error& e) {
...
ODBC_API nanodbc::result* get_database_meta_data_table_types(nanodbc::connection* conn, NativeError* error) {
	LOG_DEBUG("Getting database table types from connection: {}", reinterpret_cast<uintptr_t>(conn));
	init_error(error);
	try {
		if (!conn) {
			LOG_ERROR("Connection pointer is null, cannot get tables");
		...
		const DatabaseMetaData database_meta_data(*conn);
		auto result = database_meta_data.getTableTypes();
		auto result_ptr = new nanodbc::result(result);
		LOG_DEBUG("ResultSet created successfully: {}", reinterpret_cast<uintptr_t>(result_ptr));
		return result_ptr;
	} catch (const nanodbc::database_error& e) {
