...
public class NanodbcConnection implements Connection {
    ...
    NanodbcConnection(String dsn, String user, String password, long timeout) throws SQLException {
        try {
            connectionPtr = ConnectionHandler.connect(dsn, user, password, timeout);
            this.url = dsn;
            cleanable = cleaner.register(this, new ConnectionCleaner(connectionPtr));
        } catch (NativeException e) {
            throw new NanodbcSQLException(e);
